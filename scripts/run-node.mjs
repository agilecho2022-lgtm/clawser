...
const shouldBuild = (deps) => {
  if (deps.env.CLAWSER_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.CLAWSER_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[clawser] ${message}\n`);
};

const runClawser = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["clawser.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
