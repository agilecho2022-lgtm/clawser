...
import { markClawserExecEnv } from "../infra/clawser-exec-env.js";
...
function resolveNpmArgvForWindows(argv: string[]): string[] | null {
  ...
  if (!fs.existsSync(cliPath)) {
    // Some Windows Node installs do not place npm-cli.js next to process.execPath.
    // Fall back to npm.cmd/npx.cmd so we still route through the cmd wrapper.
...
export function resolveCommandEnv(params: {
  ...
  return markClawserExecEnv(resolvedEnv);
}
