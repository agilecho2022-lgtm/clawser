...
import type { ClawserConfig } from "../config/types.js";
...
export const DEFAULT_LOG_DIR = resolvePreferredClawserTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "clawser.log");

const LOG_PREFIX = "clawser";
...
function resolveSettings(): ResolvedSettings {
  ...
  let cfg: ClawserConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg && !shouldSkipLoadConfigFallback()) {
    try {
      const loaded = requireConfig?.("../config/config.js") as
        | {
            loadConfig?: () => ClawserConfig;
          }
        | undefined;
      ...
      if (nextBytes > settings.maxFileBytes) {
        if (!warnedAboutSizeCap) {
          ...
          process.stderr.write(
            `[clawser] log file size cap reached; suppressing writes file=${settings.file} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
        return;
      }
