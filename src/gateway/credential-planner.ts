import type { ClawserConfig } from "../config/config.js";
...
type GatewaySecretDefaults = NonNullable<ClawserConfig["secrets"]>["defaults"];

function containsEnvVarReference(value: string): boolean {
  return /\$\{[A-Z_][A-Z0-9_]*\}/i.test(value);
}

function readGatewayEnv(env: NodeJS.ProcessEnv, names: readonly string[]): string | undefined {
...
export function readGatewayPasswordEnv(env: NodeJS.ProcessEnv = process.env): string | undefined {
  return readGatewayEnv(env, ["OPENCLAW_GATEWAY_PASSWORD"]);
}
...
export function createGatewayCredentialPlan(params: {
  ...
  const localTokenCanWin = authMode !== "password" && authMode !== "none";
