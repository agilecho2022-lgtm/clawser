...
const { stopClawserChromeMock, stopChromeExtensionRelayServerMock } = vi.hoisted(() => ({
  stopClawserChromeMock: vi.fn(async () => {}),
...
vi.mock("./chrome.js", () => ({
  stopClawserChrome: stopClawserChromeMock,
}));
...
describe("stopKnownBrowserProfiles", () => {
  beforeEach(() => {
    ...
    stopClawserChromeMock.mockClear();
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopClawserChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("Clawser browser stop failed: Error: oops");
  });
});
