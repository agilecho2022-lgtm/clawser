...
function createPage(opts: { targetId: string; snapshotFull?: string; hasSnapshotForAI?: boolean }) {
  ...
  const ariaSnapshot = vi.fn().mockResolvedValue(opts.snapshotFull ?? "SNAP");
  const locator = vi.fn().mockReturnValue({ click, dblclick, fill, ariaSnapshot });
  const getByRole = vi.fn().mockReturnValue({ click, dblclick, fill });
  ...
  return { page, session, locator, getByRole, click, fill };
}
...
let rememberAriaRefsForTarget: typeof import("./pw-session.js").rememberAriaRefsForTarget;
...
beforeAll(async () => {
  ...
  ({ closePlaywrightBrowserConnection, rememberAriaRefsForTarget } = await import("./pw-session.js"));
});
...
describe("pw-ai", () => {
  ...
  it("registers aria refs from ai snapshots for act commands", async () => {
    ...
    expect(p1.getByRole).toHaveBeenCalledWith("button", { name: "OK", exact: true });
    expect(p1.click).toHaveBeenCalledTimes(1);
  });

  it("refreshes ai refs when an act command starts without cached refs", async () => {
    const snapshot = ['- button "OK"'].join("\n");
    const p1 = createPage({ targetId: "T1", snapshotFull: snapshot });
    const browser = createBrowser([p1.page]);
    ...
    expect(p1.locator).toHaveBeenCalledWith(":root");
    expect(p1.getByRole).toHaveBeenCalledWith("button", { name: "OK", exact: true });
    expect(p1.click).toHaveBeenCalledTimes(1);
  });
  ...
  it("clicks the nearest actionable ancestor for aria text refs", async () => {
    const p1 = createPage({ targetId: "T1" });
    const browser = createBrowser([p1.page]);
    (chromiumMock.connectOverCDP as unknown as ReturnType<typeof vi.fn>).mockResolvedValue(browser);
    ...
    rememberAriaRefsForTarget({
      ...
      targetId: "T1",
      nodes: [
        { ref: "ax1", role: "RootWebArea", name: "", depth: 0 },
        { ref: "ax2", role: "link", name: "Docs", depth: 1 },
        { ref: "ax3", role: "StaticText", name: "Docs", depth: 2 },
      ],
    });
    ...
    expect(p1.locator).toHaveBeenCalledWith("aria-ref=ax2");
    expect(p1.click).toHaveBeenCalledTimes(1);
  });
