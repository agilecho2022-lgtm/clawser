...
import { snapshotAriaViaPlaywright, snapshotRoleViaPlaywright } from "./pw-tools-core.snapshot.js";
...
type RestoredPage = Awaited<ReturnType<typeof getRestoredPageForTarget>>;
...
async function refLocatorWithSnapshotFallback(opts: TargetOpts, page: RestoredPage, ref: string) {
  if (/^ax\d+$/i.test(ref)) {
    await snapshotAriaViaPlaywright({
      cdpUrl: opts.cdpUrl,
      targetId: opts.targetId,
    }).catch(() => {});
    return refLocator(page, ref);
  }
  try {
    return refLocator(page, ref);
  } catch (err) {
    if (!isUnknownRefError(err, ref)) {
      throw err;
    }
    ...
    const refreshed = await getRestoredPageForTarget(opts);
    return refLocator(refreshed, ref);
  }
}
...
export async function highlightViaPlaywright(opts: {
  ...
  try {
    await (await refLocatorWithSnapshotFallback(opts, page, ref)).highlight();
  } catch (err) {
...
export async function clickViaPlaywright(opts: {
  ...
  const page = await getRestoredPageForTarget(opts);
  const ref = requireRef(opts.ref);
  const locator = await refLocatorWithSnapshotFallback(opts, page, ref);
...
export async function hoverViaPlaywright(opts: {
  ...
  try {
    await (
      await refLocatorWithSnapshotFallback(opts, page, ref)
    ).hover({
...
export async function dragViaPlaywright(opts: {
  ...
  const page = await getRestoredPageForTarget(opts);
  try {
    const startLocator = await refLocatorWithSnapshotFallback(opts, page, startRef);
    const endLocator = await refLocatorWithSnapshotFallback(opts, page, endRef);
