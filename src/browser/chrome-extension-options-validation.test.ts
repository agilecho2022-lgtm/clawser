...
const OPTIONS_VALIDATION_MODULES = [
  "../../assets/chrome-extension/options-validation.js",
  "../../chrome-extension/options-validation.js",
];

async function loadOptionsValidation(): Promise<OptionsValidationModule> {
  let lastError: unknown;
  for (const modulePath of OPTIONS_VALIDATION_MODULES) {
    try {
      return require(modulePath) as OptionsValidationModule;
    } catch (error) {
      ...
      if (message.includes("Unexpected token 'export'")) {
        return (await import(modulePath)) as OptionsValidationModule;
      }
      lastError = error;
    }
  }
...
describe("chrome extension options validation", () => {
  it("maps 401 response to relay rejected error", () => {
    const result = classifyRelayCheckResponse({ status: 401, ok: false }, 18792);
    expect(result).toEqual({
      action: "status",
      kind: "error",
      message: "Relay rejected the check request.",
    });
  });
  ...
  it("maps json response without relay status keys to wrong-port error", () => {
    const result = classifyRelayCheckResponse(
    ...
    expect(result).toEqual({
      ...
      kind: "error",
      message:
        "Wrong port: expected relay /extension/status response. Use gateway port + 3 (for gateway 18789, relay is 18792).",
    });
  });
  ...
  it("maps valid relay json response to success", () => {
    ...
    expect(result).toEqual({
      action: "status",
      kind: "ok",
      message: "Relay reachable at http://127.0.0.1:19004/",
    });
  });
  ...
  it("maps generic exceptions to relay unreachable error", () => {
    ...
    expect(result).toEqual({
      kind: "error",
      message:
        "Relay not reachable at http://127.0.0.1:18792/. Start Clawser browser relay and verify the port.",
    });
  });
});
