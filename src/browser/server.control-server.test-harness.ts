...
type HarnessState = {
  ...
  cfgProfileDriver: "openclaw" | "extension";
...
export function setBrowserControlServerProfileDriver(driver: "openclaw" | "extension"): void {
  state.cfgProfileDriver = driver;
}
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/clawser" }));
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          openclaw:
            state.cfgProfileDriver === "extension"
              ? {
                  driver: "extension",
                  cdpUrl: state.cdpBaseUrl,
                  color: "#FF4500",
                }
              : { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchClawserChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolveClawserUserDataDir: vi.fn(() => chromeUserDataDir.dir),
  stopClawserChrome: vi.fn(async () => {
...
export function getExtensionRelayMocks(): Record<string, MockFn> {
  return extensionRelayMocks as unknown as Record<string, MockFn>;
}
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgProfileDriver = "openclaw";

  mockClearAll(pwMocks);
  mockClearAll(cdpMocks);
  mockClearAll(extensionRelayMocks);
