...
import { rememberAriaRefsForTarget } from "../pw-session.js";
import {
...
export async function resolveTargetIdAfterNavigate(opts: {
  ...
  try {
    const pickReplacement = (
      ...
      pickOpts: { allowSingleTabFallback: boolean },
    ) => {
      ...
      if (pickOpts.allowSingleTabFallback && tabs.length === 1) {
        return tabs[0]?.targetId ?? opts.oldTargetId;
      }
      return opts.oldTargetId;
    };

    currentTargetId = pickReplacement(await opts.listTabs(), { allowSingleTabFallback: false });
    ...
    try {
      ...
      if (plan.format === "ai") {
        ...
        try {
          ...
          await pw
            .closePlaywrightBrowserConnection({ cdpUrl: profileCtx.profile.cdpUrl })
            .catch(() => {});
        }
      }
