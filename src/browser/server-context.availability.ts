...
export function createProfileAvailability({
  ...
  const reconcileProfileRuntime = async (): Promise<void> => {
    ...
    if (profileState.running) {
      await stopClawserChrome(profileState.running).catch(() => {});
      setProfileRunning(null);
    }
  ...
  const waitForCdpReadyAfterLaunch = async (): Promise<void> => {
    // launchClawserChrome() can return before Chrome is fully ready to serve /json/version + CDP WS.
  ...
  const ensureBrowserAvailable = async (): Promise<void> => {
    ...
    if (!httpReachable) {
      ...
      const launched = await launchClawserChrome(current.resolved, profile);
      attachRunning(launched);
      try {
        await waitForCdpReadyAfterLaunch();
      } catch (err) {
        await stopClawserChrome(launched).catch(() => {});
        setProfileRunning(null);
        throw err;
      }
      return;
    ...
    await stopClawserChrome(profileState.running);
    setProfileRunning(null);

    const relaunched = await launchClawserChrome(current.resolved, profile);
