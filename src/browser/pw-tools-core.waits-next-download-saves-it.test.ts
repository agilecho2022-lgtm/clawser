...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredClawserTmpDir: vi.fn(() => "/tmp/clawser"),
}));
vi.mock("../infra/tmp-clawser-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredClawserTmpDir.mockReturnValue("/tmp/clawser");
  });
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".clawser-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredClawserTmpDir.mockReturnValue("/tmp/clawser-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "clawser-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "clawser-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredClawserTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "clawser-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "clawser-preferred", "downloads")}${path.sep}`),
    );
  });
