...
import type { BrowserProfileConfig, ClawserConfig } from "../config/config.js";
import { loadConfig, writeConfigFile } from "../config/config.js";
import { deriveDefaultBrowserCdpPortRange } from "../config/port-defaults.js";
import { isLoopbackHost } from "../gateway/net.js";
import { resolveClawserUserDataDir } from "./chrome.js";
...
export function createBrowserProfilesService(ctx: BrowserRouteContext) {
  ...
  const createProfile = async (params: CreateProfileParams): Promise<CreateProfileResult> => {
    ...
    if (rawCdpUrl) {
      ...
      try {
        ...
        throw new BrowserValidationError(String(err instanceof Error ? err.message : err));
      }
      if (driver === "extension") {
    ...
    const nextConfig: ClawserConfig = {
      ...cfg,
      browser: {
        ...cfg.browser,
        profiles: {
  ...
  const deleteProfile = async (nameRaw: string): Promise<DeleteProfileResult> => {
    ...
    if (resolved?.cdpIsLoopback) {
      ...
      const userDataDir = resolveClawserUserDataDir(name);
