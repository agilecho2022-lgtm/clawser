...
export function resolveClawserUserDataDir(profileName = DEFAULT_OPENCLAW_BROWSER_PROFILE_NAME) {
...
export async function launchClawserChrome(
  ...
  const userDataDir = resolveClawserUserDataDir(profile.name);
  ...
  if (needsDecorate) {
    try {
      decorateClawserProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🌐 Clawser browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`Clawser browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`Clawser browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🌐 Clawser browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopClawserChrome(
