...
type StopChromeTarget = Parameters<typeof stopClawserChrome>[0];
...
describe("browser chrome profile decoration", () => {
  ...
  it("writes expected name + signed ARGB seed to Chrome prefs", async () => {
    ...
    decorateClawserProfile(userDataDir, { color: DEFAULT_OPENCLAW_BROWSER_COLOR });
  ...
  it("best-effort writes name when color is invalid", async () => {
    const userDataDir = await createUserDataDir();
    decorateClawserProfile(userDataDir, { color: "lobster-orange" });
    const def = await readDefaultProfileFromLocalState(userDataDir);
...
describe("browser chrome helpers", () => {
  ...
  it("stopClawserChrome no-ops when process is already killed", async () => {
    const proc = makeChromeTestProc({ killed: true });
    await stopChromeWithProc(proc, 10);
    expect(proc.kill).not.toHaveBeenCalled();
  });

  it("stopClawserChrome sends SIGTERM and returns once CDP is down", async () => {
  ...
  it("stopClawserChrome escalates to SIGKILL when CDP stays reachable", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue({
        ok: true,
