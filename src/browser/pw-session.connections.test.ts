...
describe("pw-session connection scoping", () => {
  ...
  it("connects Playwright over the extension relay CDP endpoint", async () => {
    const browser = makeBrowser("relay-target", "https://relay.example");
    connectOverCdpSpy.mockResolvedValue(browser.browser);
    getChromeWebSocketUrlSpy.mockResolvedValue("ws://127.0.0.1:18792/cdp?token=relay-token");

    const pages = await listPagesViaPlaywright({ cdpUrl: "http://127.0.0.1:18792" });

    expect(getChromeWebSocketUrlSpy).toHaveBeenCalledWith("http://127.0.0.1:18792", 5000);
    expect(connectOverCdpSpy).toHaveBeenCalledWith(
      "ws://127.0.0.1:18792/cdp?token=relay-token",
      expect.objectContaining({ timeout: 5000 }),
    );
    expect(pages.map((page) => page.targetId)).toEqual(["relay-target"]);
  });
});
