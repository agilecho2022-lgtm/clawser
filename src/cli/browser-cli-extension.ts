...
export function resolveBundledExtensionRootDir(
  ...
  while (true) {
    for (const candidate of [
      path.join(current, "assets", "chrome-extension"),
      path.join(current, "chrome-extension"),
    ]) {
...
export async function installChromeExtension(opts?: {
  ...
  if (!hasManifest(src)) {
    throw new Error("Bundled Chrome extension is missing. Reinstall Clawser and try again.");
  }
      ...
      defaultRuntime.error(
        info(
          [
            ...
            `- Pin “Clawser Browser Relay”, then click it on the tab (badge shows ON)`,
          ].join("\n"),
        ),
      );
    });
    ...
    .action(async (_opts, cmd) => {
      ...
      if (!hasManifest(dir)) {
        defaultRuntime.error(
          danger(
            [
              `Clawser Chrome extension is not installed. Run: "clawser extension install"`,
            ].join("\n"),
          ),
        );
        defaultRuntime.exit(1);
