...
export function formatCliBannerLine(version: string, options: BannerOptions = {}): string {
  ...
  const richCommitSuffix = commit ? ` ${theme.muted(`(${commit})`)}` : "";
  const plainBaseLine = `${title} ${version}${commitSuffix}`;
  const plainFullLine = tagline ? `${plainBaseLine} — ${tagline}` : plainBaseLine;
  const fitsOnOneLine = visibleWidth(plainFullLine) <= columns;
  if (rich) {
    if (fitsOnOneLine) {
      if (!tagline) {
        return `${theme.heading(title)} ${theme.info(version)}${richCommitSuffix}`;
      }
      return `${theme.heading(title)} ${theme.info(version)}${richCommitSuffix} ${theme.muted(
        "—",
      )} ${theme.accentDim(tagline)}`;
    }
    const line1 = `${theme.heading(title)} ${theme.info(version)}${richCommitSuffix}`;
    if (!tagline) {
      return line1;
    }
