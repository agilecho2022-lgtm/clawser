...
async function runBrowserToggle(
  ...
  defaultRuntime.log(info(`🌐 Clawser browser [${name}] running: ${status.running}`));
}
  ...
  browser
    ...
    .action(async (_opts, cmd) => {
      ...
      await runBrowserCommand(async () => {
        ...
        if (!result.moved) {
          defaultRuntime.log(info(`🌐 Clawser browser profile already missing.`));
          return;
        }
        const dest = result.to ?? result.from;
        defaultRuntime.log(info(`🌐 Clawser browser profile moved to Trash (${dest})`));
      });
    });
    ...
    .action(
      async (opts: { name: string; color?: string; cdpUrl?: string; driver?: string }, cmd) => {
        ...
        await runBrowserCommand(async () => {
          ...
          defaultRuntime.log(
            info(
              `🌐 Clawser created profile "${result.profile}"\n${loc}\n  color: ${result.color}${
    ...
    .action(async (opts: { name: string }, cmd) => {
      ...
      await runBrowserCommand(async () => {
        ...
        const msg = result.deleted
          ? `🌐 Clawser deleted profile "${result.profile}" (user data removed)`
          : `🌐 Clawser deleted profile "${result.profile}" (no user data found)`;
